...
for subject in scz_subjects:
    ...
    np.save(f'{scz_path}/sub-{subject}/func/disp_sing{n_grads}_{n_neighbours}n_{subject}_schaefer1000.npy', subj_disp)
...
for subject in control_subjects:
    ...
    distances, indices = hcp_ngbrs.kneighbors(gradients)
    subj_disp = distances.mean(axis = 1)
    np.save(f'{controls_path}/sub-{subject}/func/disp_sing{n_grads}_{n_neighbours}n_{subject}_schaefer1000.npy', subj_disp)