...
print(f"CV done for all models for {n_features} features.")
performance["N features"] = n_features


if os.path.exists(f"results/model_results_cv.csv"):
    model_results = pd.read_csv(f"results/model_results_cv.csv")
    model_results = pd.concat([model_results, performance], axis = 0)
    model_results.to_csv(f"results/model_results_cv.csv", index = False)
else:
    performance.to_csv(f"results/model_results_cv.csv", index = False)
